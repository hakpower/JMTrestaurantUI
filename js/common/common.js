var apiHostname = "http://" + window.location.hostname + ":8080";
...
function showAlert(msg, status, cb = null) {
  ...
  $("#header").append(
    '<div class="alertContainer"><span class="msg">' + msg + "</span></div>"
  );
  $("#header>.alertContainer").css("opacity", "1");
  switch (status) {
    case 1:
      $("#header>.alertContainer").css(
        "background",
        "linear-gradient(to top, white 1%, #05ef6db8 20%)"
      );
      break;
    case 2:
      $("#header>.alertContainer").css(
        "background",
        "linear-gradient(to top, white 1%, #f90000b8 20%)"
      );
      break;
    default:
      $("#header>.alertContainer").css(
